...
const Signup = () => {
    ...
    return (<><center>
        ...
        <div className='flex flex-col justify-center bg-slate-600 items-center m-auto  p-5 w-full sm:w-4/5 md:w-4/6 lg:w-2/5  border rounded-md ' >
            ...
            <select
            className="text-yellow-700 bg-slate-600"
              name="Role"
              value={role}
              onChange={(e) => {
        ...
        <button className='bg-blue-600 rounded-md font-sans mt-12' onClick={checkpassword}>Register</button>
        <div className='flex justify-between m-5'>
               <div> <p className='text-gray-900'>Already have an account?</p></div>
                <div><Link to="/login" className='text-blue-400'>Login</Link></div>
            </div>
            </div>
        </div>
